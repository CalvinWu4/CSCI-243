...
char *songGetArtist(const song *s) 
{
    ...
    strncpy(tmp, s->artist, strlen(s->artist)+1) ;
    free(artist)
    return tmp ;
}
...
char *songGetTitle(const song *s)
{
    ...
    strncpy(tmp, s->title, strlen(s->title)+1) ;
    free(title)
    return tmp ;
}
...
song *songCopy(const song *s)
{
    ...
    strncpy(d->artist, s->artist, sizeof(d->artist)+1) ;

    d->title = malloc(strlen(s->title)+1) ;
    strncpy(d->title, s->title, sizeof(d->artist)+1) ;
